...
#include "8P.h"
#include "7T.h"
//!  An 8P is a 7T with an additional particle
/*!
* A 7T cluster contains two 6Z clusters at the top and bottom of the cluster. 
* An 8P takes these two 6Z clusters and tries to make a 7T from the cluster and an extra particle.
...
void Clusters_Get8P() {
    ...
    int clust_6Z[7][2]; 
    clust_6Z[0][0] = old_7T_cluster[3];
    clust_6Z[1][0] = old_7T_cluster[2];
    clust_6Z[2][0] = old_7T_cluster[5];
    clust_6Z[3][0] = old_7T_cluster[6];
    clust_6Z[4][0] = old_7T_cluster[0];
    clust_6Z[5][0] = old_7T_cluster[4];
    clust_6Z[6][0] = old_7T_cluster[1];
    
    clust_6Z[0][1] = old_7T_cluster[2];
    clust_6Z[1][1] = old_7T_cluster[3];
    clust_6Z[2][1] = old_7T_cluster[0];
    clust_6Z[3][1] = old_7T_cluster[1];
    clust_6Z[4][1] = old_7T_cluster[5];
    clust_6Z[5][1] = old_7T_cluster[4];
    clust_6Z[6][1] = old_7T_cluster[6];
                        ...
                        if(is_particle_ring_of_5A(spindle_id,new_5A_cluster) == 1){
                            int new_particle_id = get_new_particle(new_5A_cluster, spindle_id_2);
                                ...
                                int bond_counter = check_ring_bonds(new_5A_cluster, arr_6Z);
                                    if (bond_counter == 25 ){
                                        if (check_unique8P(old_7T_cluster, new_particle_id,8) == 0){
                                        add_8P(bonded_id, nbonded_id_2, bonded_id_2, nbonded_id,spindle_id, spindle_id_2, final_part1, new_particle_id);
                                        }
                                    }
                                }
                            }
...
// Given a 6Z and an 8P, returns the number of common particles
...
void add_8P(int bonded_id, int nbonded_id_2, int bonded_id_2, int nbonded_id, int spindle_id, int spindle_id_2, int final_part, int new_particle) {
    int clusSize = 8;
    if (n8P == m8P) {
        hc8P = resize_2D_int(hc8P, m8P, m8P + incrStatic, clusSize, -1);
        m8P = m8P + incrStatic;
    }
    hc8P[n8P][0] = bonded_id;
    hc8P[n8P][1] = nbonded_id_2;
    hc8P[n8P][2] = bonded_id_2;
    hc8P[n8P][3] = nbonded_id;
    hc8P[n8P][4] = spindle_id;
    hc8P[n8P][5] = spindle_id_2;
    hc8P[n8P][6] = final_part;
...
int check_unique8P(const int *old_clust, int new_particle, int new_clust_size){
    int u;
    for (int old_8P_id = 0; old_8P_id < n8P; ++old_8P_id) {
        u = 0;
        for (int q = 0; q<new_clust_size; ++q){
            for (int r = 0; r<7; ++r){
                if(hc8P[old_8P_id][q] == old_clust[r]){
                    u += 1;
                }
            }
